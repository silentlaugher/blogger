	...
	metaSaveBtn.addEventListener("click", function(event){
		metaText  = document.querySelector("#metaDescInput");
 		if(metaText.value.trim().length < '150'){
			...
			formData.append("description", metaText.value.trim());
			formData.append("blogID", blogID);
 
			var httpRequest = new XMLHttpRequest();
 
			if(httpRequest){
				httpRequest.open('POST', 'http://localhost/blogger/backend/ajax/updateMeta.php', true);
			...
			document.querySelector("#metaDescError").innerHTML = "Must be 500 characters or less!";
		}
	});
});