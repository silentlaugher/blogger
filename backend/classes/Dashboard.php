<?php 
    class Dashboard{
        ...
        public function getAllComments($offset, $limit, $type,$blogID){
			...
			$sql = "SELECT * FROM `users` `U`,`comments` `C` LEFT JOIN `posts` `P` ON `P`.`postID` = `C`.`postID` WHERE `U`.`userID` = `P`.`authorID` AND `C`.`blogID` = :blogID AND `C`.`status` = :type ORDER BY `commentID` DESC LIMIT :offset, :postLimit";
