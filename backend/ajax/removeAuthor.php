<?php 
	...
	if($_SERVER['REQUEST_METHOD'] === "POST"){
		if(isset($_POST['blogID'])){
			...
			if($blog){
				if($blog->role !== "Admin"){
					echo "Error, You don't have the rights to delete this user!";
				}else{
					if($blog->CreatedBy  === $author->userID){
						echo "Error, Blog Creator can't be removed!";
					}else{
						if($blog->userID !== $author->userID){
						  $userObj->delete('users', ['userID' => $author->userID]);
						  $userObj->delete('blogsAuth', ['userID' => $author->userID]);
						}else{
							echo 'Error You can\'t remove yourself from the author list!';
						}
					}
				}
			}
